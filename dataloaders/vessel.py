...
class VesselDataset(BaseDataSet):
    def __init__(self, **kwargs):
        ...
        super(VesselDataset, self).__init__(**kwargs)
...
class Vessel(BaseDataLoader):
    def __init__(self, data_dir, batch_size, split, square_resize=None, crop_size=None, base_size=None, scale=True, num_workers=1, val=False,
        ...
        if split in ["train", "trainval", "val", "test"]:
            self.dataset = VesselDataset(**kwargs)
        else: raise ValueError(f"Invalid split name {split}")
        super(Vessel, self).__init__(self.dataset, batch_size, shuffle, num_workers, val_split)
