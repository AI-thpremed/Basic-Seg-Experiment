...
class LayerDataset(BaseDataSet):
    """
    Pascal Voc dataset
    ...
    def __init__(self, **kwargs):
        ...
        super(LayerDataset, self).__init__(**kwargs)

    def _set_files(self):
        self.root = os.path.join(self.root, '/vepfs/niuzhiyuan/python/fluidseg/data/layer')
        self.image_dir = os.path.join(self.root, 'Image')
        self.label_dir = os.path.join(self.root, 'Layer_Masks')

        file_list = os.path.join(self.root, "txt", self.split + ".txt")
...
class Layer(BaseDataLoader):
    def __init__(self, data_dir, batch_size, split, square_resize=None, crop_size=None, base_size=None, scale=True, num_workers=1, val=False,
        ...
        if split in ["train", "trainval", "val", "test"]:
            self.dataset =LayerDataset(**kwargs)
        else: raise ValueError(f"Invalid split name {split}")
        super(Layer, self).__init__(self.dataset, batch_size, shuffle, num_workers, val_split)
