...
class DISCDataset(BaseDataSet):
    def __init__(self, **kwargs):
        self.num_classes = 2
        self.palette = palette.get_voc_palette(self.num_classes)
        super(DISCDataset, self).__init__(**kwargs)

    def _set_files(self):
        self.root = os.path.join(self.root, 'disc')
...
class DISC(BaseDataLoader):
    def __init__(self, data_dir, batch_size, split, square_resize=None, crop_size=None, base_size=None, scale=True, num_workers=1, val=False,
        ...
        if split in ["train", "trainval", "val", "test"]:
            self.dataset = DISCDataset(**kwargs)
        else: raise ValueError(f"Invalid split name {split}")
        super(DISC, self).__init__(self.dataset, batch_size, shuffle, num_workers, val_split)
