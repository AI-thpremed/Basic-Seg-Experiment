...
class DISCCUPDataset(BaseDataSet):
    def __init__(self, **kwargs):
        self.num_classes = 3
        self.palette = palette.get_voc_palette(self.num_classes)
        super(DISCCUPDataset, self).__init__(**kwargs)

    def _set_files(self):
        self.root = os.path.join(self.root, 'disc_cup')
...
class DISCCUP(BaseDataLoader):
    def __init__(self, data_dir, batch_size, split, square_resize=None, crop_size=None, base_size=None, scale=True, num_workers=1, val=False,
        ...
        if split in ["train", "trainval", "val", "test"]:
            self.dataset = DISCCUPDataset(**kwargs)
        else: raise ValueError(f"Invalid split name {split}")
        super(DISCCUP, self).__init__(self.dataset, batch_size, shuffle, num_workers, val_split)
