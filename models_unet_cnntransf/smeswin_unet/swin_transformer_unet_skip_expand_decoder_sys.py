...
## new
        self.mcct = ChannelTransformer(config_vit, img_size=112, patchSize=[112, 56, 28, 14], channel_num=[48, 96, 192, 384])
        #self.cct = ChannelTransformer(config_vit, img_size=56, patchSize=[56, 28, 14, 7],channel_num=[96, 192, 384, 768])
        channeld1 = 96
        channeld2 = 192
        channeld3 = 384

        self.cnnt1 = torch.nn.Conv2d(in_channels=3, out_channels=48, kernel_size=3, stride=2, padding=1, dilation=1, groups=1, bias=True, padding_mode='zeros')

        self.EA_channeld3 = ExternalAttention(d_model = channeld3, S=8)
        self.EA_channeld2 = ExternalAttention(d_model = channeld2, S=8)
        self.EA_channeld1 = ExternalAttention(d_model = channeld1, S=8)
        self.cnn4supp = torch.nn.Conv2d(in_channels=self.num_classes+3, out_channels=self.num_classes,
    ...
    # Reshape from transformer to cnn
    def skipshape(self, x):
        B, L, C = x.shape
        H, W = int(pow(L, 1/2)), int(pow(L, 1/2))
        assert L == H*W, "input features has wrong size"
        x = x.view(B,H,W,-1) #4*H,4*W
        x = x.permute(0,3,1,2) #B,C,H,W
        return x

    # reshape from cnn to transformer
    def rev_skipshape(self, x):
